...
2. Help is generated by the `generateNodeHelp()` function which:
   ...
   - Attempts to observe `oneditprepare()` to discover form fields
...
- **Inputs**: Discovers inputs from common properties in locale files
- **Fields**: Would observe form builder in `oneditprepare()` to discover fields (currently using fallback)
